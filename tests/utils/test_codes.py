...
@pytest.mark.parametrize(
    "year,seq,expected",
    [
        (2025, 1, "PACKAGE-2025-000001"),
        (2025, 42, "PACKAGE-2025-000042"),
        (2025, 123456, "PACKAGE-2025-123456"),
        (2025, 999999, "PACKAGE-2025-999999"),
        (1999, 7, "PACKAGE-1999-000007"),
    ],
)
...
def test_build_tracking_code_zero_seq():
    ...
    assert build_tracking_code(2025, 0) == "PACKAGE-2025-000000"


@freeze_time('2033-05-04')
def test_generate_tracking_code_uses_current_year():
    assert generate_tracking_code(15) == "PACKAGE-2033-000015"
...
def test_build_tracking_code_negative_seq(bad_seq):
    ...
    expected = f"PACKAGE-2025--{abs(bad_seq):05d}" if bad_seq < 0 else ""
